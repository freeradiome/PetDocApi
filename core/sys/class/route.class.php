...
class Route{
	...
	private function get_object_params_by_query(){
		...
		$class = trim(isset($_GET[$this->_class_query])?$_GET[$this->_class_query]:'');
		...
		$method = trim(isset($_GET[$this->_method_query])?$_GET[$this->_method_query]:'');
		if(!$method){
		...
		if(empty($class)){
			throw_exception('对象不能为空');
		}
		if(empty($method)){
			throw_exception('方法能为空');
		}
