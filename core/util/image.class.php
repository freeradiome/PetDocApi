...
class Image{
	...
	public $allow_type =array("image/gif","image/pjpeg","image/jpeg","image/*","image/x-png","image/png");
	//缩略参数
	...
	public function ImageResize(){
		...
		if($_FILES[$this->input_name]['size']<$this->file_size){
			...
			if(in_array($_FILES[$this->input_name]['type'],$this->allow_type)  ){
				...
				$srcfile = $_FILES[$this->input_name]['tmp_name'];
		...
		if($srcfile){
			...
			$tofile = $this->save_path.$this->savefile_name; //保存为新文件名
			$info = "";
